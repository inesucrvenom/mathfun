"""Test for recursive factorial function on AWS Lambda.
...
import unittest
from test.test_lambda.run_lambda import invoke_lambda


def call_lambda(val: int) -> int:
    """Convert local call into more abstract one from manual_run"""
    result = invoke_lambda('lambda_factorial_recursive', {'n': val})
    return result
...
class Test_FactorialRecursive_Time(unittest.TestCase):
    ...
    def test_time_980(self):
        with self.assertRaises(Exception): call_lambda(995)
