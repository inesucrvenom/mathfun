"""Test for recursive factorial function"""

import unittest
from src.lambda_factorial_recursive import factorial


class Test_FactorialRecursive_ValidInput(unittest.TestCase):
    """Test correctness on valid input."""

    def test_correctness_valueForSmallIndices(self):
        self.assertEqual(1, factorial(0))
        self.assertEqual(1, factorial(1))
        self.assertEqual(2, factorial(2))
        self.assertEqual(6, factorial(3))
        self.assertEqual(24, factorial(4))
        self.assertEqual(120, factorial(5))
        self.assertEqual(720, factorial(6))
        self.assertEqual(3628800, factorial(10))
        self.assertEqual(25852016738884976640000, factorial(23))

    def test_correctness_differentIntegerRepresentation_forInt18(self):
        self.assertEqual(6402373705728000, factorial(0x12))
        self.assertEqual(6402373705728000, factorial(0o22))
        self.assertEqual(6402373705728000, factorial(0b10010))

    def test_correctness_fakeTupleOfOne(self):
        # this is not a tuple, but int; tuple is defined by comma
        self.assertEqual(24, factorial((4)))


class Test_FactorialRecursive_InvalidInput(unittest.TestCase):
    """Test of handling exceptions sent through Fibonnaci."""

    def test_wrongValue_negativeIntegers(self):
        with self.assertRaises(ValueError): factorial(-2)

    def test_wrongType_lookLikeInteger(self):
        with self.assertRaises(TypeError): factorial(3.0)
        with self.assertRaises(TypeError): factorial(0.0)
        with self.assertRaises(TypeError): factorial(-23.0)

    def test_wrongType_obviousFloat(self):
        with self.assertRaises(TypeError): factorial(3.5)
        with self.assertRaises(TypeError): factorial(-2.5)

    def test_wrongType_string(self):
        with self.assertRaises(TypeError): factorial("a")

    def test_wrongType_array(self):
        with self.assertRaises(TypeError): factorial([])
        with self.assertRaises(TypeError): factorial([4])
        with self.assertRaises(TypeError): factorial([4, 3])

    def test_wrongType_dictionary(self):
        with self.assertRaises(TypeError): factorial({})
        with self.assertRaises(TypeError): factorial({10: 40})

    def test_wrongType_tuple(self):
        with self.assertRaises(TypeError): factorial(())
        with self.assertRaises(TypeError): factorial((1,))
        with self.assertRaises(TypeError): factorial((2, 3))


class Test_FactorialRecursive_Time(unittest.TestCase):
    """Test correctness on valid input, or check time/timeout."""

    def test_time_30(self):
        self.assertEqual(265252859812191058636308480000000,
                         factorial(30))

    def test_time_100(self):
        self.assertEqual(93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000,
                         factorial(100))

    def test_time_500(self):
        self.assertEqual(,
                         factorial(500))

    def test_time_980(self):
        with self.assertRaises(RecursionError): factorial(980)


if __name__ == '__main__':
    unittest.main()
